...
namespace MVCData.Helpers.SqlProgrammability.StockTasks
{
    class Class1
    {
        ...
        public string GET_WarehouseJournal_BUILD_SQL(string warehouseJournalTable, string fromDate, string toDate, string warehouseIDList, string commodityIDList, string isFullJournal)
        {
            string queryString = "              DECLARE     " + warehouseJournalTable + " TABLE \r\n";

            queryString = queryString + "                  (NMVNTaskID int NOT NULL, JournalPrimaryID int NOT NULL, JournalDate DateTime NOT NULL, JournalReference nvarchar(50) NOT NULL, JournalDescription nvarchar(100) NULL, " + "\r\n";
            ...
            queryString = queryString + "       INSERT INTO " + warehouseJournalTable + " EXEC WarehouseJournal " + fromDate + ", " + toDate + ", " + warehouseIDList + ", " + commodityIDList + ", " + isFullJournal;
        ...
        {
            string queryString = " @FromDate DateTime, @ToDate DateTime, @WarehouseIDList varchar(35), @CommodityIDList varchar(3999), @isFullJournal bit " + "\r\n";
            queryString = queryString + " WITH ENCRYPTION " + "\r\n";
            queryString = queryString + " AS " + "\r\n";
        ...
        private string WarehouseJournalBuildSQLA(bool isFullJournal, bool isAmountIncluded, bool isWarehouseFilter, bool isCommodityFilter)
        {
            ...
            queryString = queryString + "                       GROUP BY        NMVNTaskID, JournalPrimaryID, CommodityID, WarehouseID " + "\r\n";
        ...
        private void VehicleCard()
        {
            ...
            queryString = queryString + "                   SELECT     'HÀNG TỒN KHO' AS GroupName, 'TỒN ' + CONVERT(VARCHAR, @ToDate, 103) AS SubGroupName, GoodsReceiptDetails.EntryDate, GoodsReceiptDetails.CommodityID, GoodsReceiptDetails.ChassisCode, GoodsReceiptDetails.EngineCode, GoodsReceiptDetails.ColorCode, GoodsReceiptDetails.WarehouseID, 'VCNB: ' + Warehouses.Name AS Description, 0 AS QuantityDebit, StockTransferDetails.Quantity AS QuantityCredit " + "\r\n";
            queryString = queryString + "                   FROM        StockTransferDetails INNER JOIN " + "\r\n";
            ...
            queryString = queryString + "                   SELECT     'XE TRÊN ĐƯỜNG VỀ' AS GroupName, 'CHỜ NHẬP KHO' AS SubGroupName, PurchaseInvoices.EntryDate, GoodsReceiptDetails.CommodityID, '' AS ChassisCode, '' AS EngineCode, '' AS ColorCode, 0 AS WarehouseID, Suppliers.Name + ', HĐ [' + ISNULL(PurchaseInvoices.VATInvoiceNo, '') + ' Ngày: ' + CONVERT(VARCHAR, PurchaseInvoices.EntryDate, 103) + ']' AS Description, GoodsReceiptDetails.Quantity AS QuantityDebit, 0 AS QuantityCredit " + "\r\n";
            queryString = queryString + "                   FROM        PurchaseInvoices INNER JOIN " + "\r\n";
            ...
            queryString = queryString + "                   SELECT     'XE TRÊN ĐƯỜNG VỀ' AS GroupName, 'CHỜ NHẬP KHO' AS SubGroupName, StockTransfers.EntryDate, GoodsReceiptDetails.CommodityID, '' AS ChassisCode, '' AS EngineCode, '' AS ColorCode, 0 AS WarehouseID, 'VCNB: ' + Locations.Name + ', PX [' + StockTransfers.Reference + ' Ngày: ' + CONVERT(VARCHAR, StockTransfers.EntryDate, 103) + ']' AS Description, GoodsReceiptDetails.Quantity AS QuantityDebit, 0 AS QuantityCredit " + "\r\n";
            queryString = queryString + "                   FROM        StockTransfers INNER JOIN " + "\r\n";
